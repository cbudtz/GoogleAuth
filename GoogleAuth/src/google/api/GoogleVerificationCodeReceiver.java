...
public final class GoogleVerificationCodeReceiver implements VerificationCodeReceiver {
   ...
   */
  public GoogleVerificationCodeReceiver(String host, int port) {
    this.host = host;
    this.port = port;
  }
  ...
  public String waitForCode() throws IOException {
    ...
    try {
      while (code == null && error == null) {
        gotAuthorizationResponse.await(60L, TimeUnit.SECONDS);
        if (code == null) throw new IOException("Received no Code!");
      }
      if (error != null) {
      ...
      return code;
		...
		throw new IOException("Authorization took too long!");
	} finally {
      lock.unlock();
    }
  }
  ...
  public static final class Builder {
    ...
    /** Builds the {@link GoogleVerificationCodeReceiver}. */
    public GoogleVerificationCodeReceiver build() {
      return new GoogleVerificationCodeReceiver(host, port);
    }
  ...
  class CallbackHandler extends AbstractHandler {
    ...
    private void writeLandingHtml(HttpServletResponse response) throws IOException {
      ...
      doc.println("<body>");
      doc.println("Received OAuth token. You may now close this window...");
      doc.println("</body>");
      doc.println("</HTML>");
      doc.flush();
    }
